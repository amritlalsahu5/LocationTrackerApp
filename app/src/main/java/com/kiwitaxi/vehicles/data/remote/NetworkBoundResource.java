 ...
 * @description This class act as the decider to cache the response/ fetch from the service always
...
public abstract class NetworkBoundResource<T, V> {
    ...
    private String getCustomErrorMessage(Throwable error){
        ...
        if (error instanceof SocketTimeoutException) {
            return KiwiTaxiVehicleApp.getAppContext().getString(R.string.requestTimeOutError);
        } else if (error instanceof MalformedJsonException) {
            return  KiwiTaxiVehicleApp.getAppContext().getString(R.string.responseMalformedJson);
        } else if (error instanceof IOException) {
             return  KiwiTaxiVehicleApp.getAppContext().getString(R.string.networkError);
        } else if (error instanceof HttpException) {
            return (((HttpException) error).response().message());
        } else {
            return KiwiTaxiVehicleApp.getAppContext().getString(R.string.unknownError);
        }
